...
def _full_hash_path(path: str) -> str:
    """SHA-256 hash every component; keep .py suffix only."""
    parts = []
    for x in path.split("/"):
        if not x:
            continue
...
def train(args):
    ...
    for epoch in range(args.num_epochs):
        ...
        for ex_idx, data_idx in enumerate(indices):
            ...
            # Always use code-aware prompts (consistency regularization needs code)
            codes = [_read_code_content(args.repo_dir, repo, c, args.code_max_lines)
                     for c in candidates]
            prompts_real = [build_prompt_with_code(issue_text, c, code)
                            for c, code in zip(candidates, codes)]
            # Consistency branch: same code, hashed path
            prompts_hash = [build_prompt_with_code(issue_text, _full_hash_path(c), code)
                            for c, code in zip(candidates, codes)]
            prompts = prompts_real  # primary forward still uses real path
            ...
            # Consistency branch: hashed-path scores, same group
            if args.consistency_lambda > 0:
                try:
                    scores_hash = compute_scores(
                        model, tokenizer, prompts_hash,
                        yes_id, no_id,
                        args.max_seq_length, device,
                    )
                    log_probs_hash = F.log_softmax(scores_hash, dim=0)
                    # Symmetric KL between real and hashed distributions
                    p_real = log_probs.exp()
                    p_hash = log_probs_hash.exp()
                    kl_r2h = (p_real * (log_probs - log_probs_hash)).sum()
                    kl_h2r = (p_hash * (log_probs_hash - log_probs)).sum()
                    cons_loss = 0.5 * (kl_r2h + kl_h2r)
                    # Also push hashed-path positive to be correct (code-only supervision)
                    ce_hash = -log_probs_hash[0]
                    loss = ce_loss + args.consistency_lambda * cons_loss + args.hash_ce_weight * ce_hash
                except RuntimeError as e:
                    if "out of memory" in str(e).lower():
                        torch.cuda.empty_cache()
                        loss = ce_loss
            ...
            if (ex_idx + 1) % args.gradient_accumulation_steps == 0:
                ...
                if global_step % args.logging_steps == 0:
                    ...
                    if (args.path_augment_fraction > 0
...
def main():
    ...
    parser.add_argument("--consistency_lambda", type=float, default=1.0,
                        help="Weight on symmetric KL between real-path and hashed-path rankings.")
    parser.add_argument("--hash_ce_weight", type=float, default=0.5,
                        help="Weight on CE loss computed on hashed-path pass (code-only supervision).")
