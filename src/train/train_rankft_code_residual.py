...
# ============================================================
# Prompt template — Code-Residual variant
# Paths are anonymized, code content is included.
# ============================================================
...
# Global state for code loading
_REPO_BASE_DIR = "data/repos"
_CODE_MAX_LINES = 100
_ANONYMIZE_PATHS = True

# Per-example anonymization map: reset each example to prevent
# counter-based ordering from leaking BM25 rank information.
# Key: original path -> Value: anonymized ID
_ANON_MAP = {}
_ANON_RNG = random.Random(42)


def reset_anonymization_for_example(candidate_paths: list):
    """Reset anonymization map for a new example.

    Shuffles candidate order before assigning IDs so that
    file_0001.py is NOT always the BM25 top-1 candidate.
    This prevents the model from learning rank-order shortcuts.
    """
    global _ANON_MAP
    _ANON_MAP = {}
    shuffled = list(candidate_paths)
    _ANON_RNG.shuffle(shuffled)
    for i, path in enumerate(shuffled):
        _ANON_MAP[path] = f"file_{i % 10000:04d}.py"


def _read_file_content(repo_dir: str, file_path: str, max_lines: int = 100) -> str:
...
def _find_repo_dir(repo_name: str) -> str:
    """Map repo name like 'astropy/astropy' to data/repos/astropy."""
    candidates = [
...
# Current repo context (set per-example in training loop)
_current_repo = [""]


def build_prompt(issue_text: str, candidate_path: str) -> str:
    """Build prompt with code content and anonymized path."""
    # Anonymize path using per-example shuffled map
    if _ANONYMIZE_PATHS:
        assert candidate_path in _ANON_MAP, \
            f"Path '{candidate_path}' not in anonymization map. Call reset_anonymization_for_example() first."
        display_path = _ANON_MAP[candidate_path]
    else:
        display_path = candidate_path

    # Load code content
    repo_dir = _find_repo_dir(_current_repo[0])
    if repo_dir:
        code = _read_file_content(repo_dir, candidate_path, _CODE_MAX_LINES)
    else:
    ...
    return PROMPT_TEMPLATE.format(
        ...
        code_content=code[:6000],  # Allow enough chars for 100-line ablation
    )
...
def train(args):
    ...
    for epoch in range(args.num_epochs):
        ...
        for ex_idx, data_idx in enumerate(indices):
            ...
            repo = example["repo"]
            _current_repo[0] = repo  # Set for code-residual prompt building
            ...
            # Reset anonymization map for this example with shuffled order
            # so file_0001.py is NOT always the positive/BM25-top candidate.
            # Use original paths (before any delex mutation) for both
            # anonymization lookup and code content loading.
            original_candidates = list(candidates)
            if _ANONYMIZE_PATHS:
                reset_anonymization_for_example(original_candidates)

            # On-the-fly delexicalization for path debiasing (path-only model concern).
            # For code-residual model, we always use original paths for build_prompt
            # since paths are already anonymized and code is loaded from disk.
            # Delex only affects the base train_rankft.py path-only prompt.
            # Note: delex_fraction is ignored for code-residual model since
            # paths are already anonymized. Delex only affects path-only prompts.

            prompts = [build_prompt(issue_text, cand) for cand in original_candidates]
...
def main():
    ...
    # Code-residual specific args
    parser.add_argument("--repo_dir", type=str, default="data/repos",
                        help="Directory containing repo checkouts for code content loading")
    parser.add_argument("--code_max_lines", type=int, default=100,
                        help="Max lines of code to include per file")
    parser.add_argument("--anonymize_paths", action="store_true", default=True,
                        help="Anonymize file paths (for code-residual training)")
    parser.add_argument("--no_anonymize_paths", action="store_false", dest="anonymize_paths")

    args = parser.parse_args()

    # Set global code-residual config
    global _REPO_BASE_DIR, _CODE_MAX_LINES, _ANONYMIZE_PATHS
    _REPO_BASE_DIR = args.repo_dir
    _CODE_MAX_LINES = args.code_max_lines
    _ANONYMIZE_PATHS = args.anonymize_paths
    print(f"  Code-residual mode: anonymize={_ANONYMIZE_PATHS}, "
          f"code_max_lines={_CODE_MAX_LINES}, repo_dir={_REPO_BASE_DIR}")
