"""
RankFT Code-Residual Long-Context: Reranker with anonymized paths + code at 4096 context.

Experiment 1C variant. Key differences from code-centric (1A):
  - Path anonymization via MD5 hash -> model must rely on code content, not path tokens
  - Longer context (4096 tokens) to fit more code
  - More code per file (head_lines=300, max_chars=12000)
  - Fewer negatives (2) to fit in GPU memory at longer context
  - Higher gradient accumulation (32) to compensate

Path anonymization:
  - Each file path is replaced with md5(path)[:12] + original extension
  - Hash assignments are shuffled per-example to prevent BM25 rank leakage
  - Different examples can reuse same hash mapping independently

Usage:
    python src/train/train_rankft_code_residual_longctx.py \
        ...
        --repo_dir data/repos \
        --output_dir /data/chenlibin/grepo_agent_experiments/code_residual_longctx \
        --device cuda:0
"""
...
# ============================================================
# Prompt template — Code-Residual Long-Context variant
# Anonymized paths + code content. Model must rely on code, not paths.
# ============================================================
...
# ============================================================
# Path anonymization via MD5 hash
# ============================================================

_ANON_RNG = random.Random(42)


def anonymize_paths_for_example(candidate_paths: List[str]) -> Dict[str, str]:
    """Create an MD5-based anonymization map for one example's candidate set.

    Each file path -> md5(path)[:12] + original extension.
    Hash assignments are shuffled to prevent BM25 rank leakage:
    we assign hashes in shuffled order so that the positive file
    does not always get the same relative hash position.

    Returns {original_path: anonymized_path}.
    """
    # Generate (path, hash) pairs
    path_hashes = []
    for path in candidate_paths:
        ...
        if not ext:
            ext = ".py"
        path_hashes.append((path, f"{h}{ext}"))

    # Shuffle the hash assignments to break ordering correlation
    # We keep the original paths but shuffle which hash goes to which path
    hashes = [h for _, h in path_hashes]
    _ANON_RNG.shuffle(hashes)

    anon_map = {}
    for i, path in enumerate(candidate_paths):
        anon_map[path] = hashes[i]

    return anon_map


def build_prompt(issue_text: str, anon_path: str, code_content: str,
                 tokenizer=None, max_seq_length: int = 4096) -> str:
...
# (reused from code-centric script)
# ============================================================
...
def train(args):
    """Main training routine -- code-residual long-context variant."""
    ...
    # ---- Training loop ----
    print(f"\n{'='*60}")
    print(f"RankFT Code-Residual Long-Context Training (Exp 1C)")
    ...
    print(f"  Path anonymization: MD5 hash (shuffled per-example)")
    ...
    for epoch in range(args.num_epochs):
        ...
        for ex_idx, data_idx in enumerate(indices):
            ...
            # Build candidates list and anonymize paths
            candidates = [positive_file] + negatives
            anon_map = anonymize_paths_for_example(candidates)

            # Build prompts with anonymized paths and code content
            prompts = []
...
def main():
    parser = argparse.ArgumentParser(
        description="RankFT Code-Residual Long-Context (Exp 1C): "
                    "reranker with MD5-anonymized paths + code at 4096 context"
    )
    ...
    parser.add_argument("--code_head_lines", type=int, default=300,
                        help="Number of head lines to read from each file")
    parser.add_argument("--code_max_chars", type=int, default=12000,
                        help="Max chars of code content (~3000 tokens)")
    ...
    parser.add_argument("--num_negatives", type=int, default=2,
                        help="Negatives per positive (reduced for memory at 4096 ctx)")
    ...
    parser.add_argument("--max_seq_length", type=int, default=4096,
                        help="Max tokens per prompt (long context for code)")
