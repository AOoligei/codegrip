...
CUDA_VISIBLE_DEVICES=$GPU $PY -u scripts/eval_codeaware_4bit.py \
    ...
    --gpu_id 0 --top_k 100 --max_seq_length 768 --score_batch_size 8 2>&1 | tee logs/eval_pathswap_${TAG}_normal.log | tail -3
...
CUDA_VISIBLE_DEVICES=$GPU $PY -u scripts/eval_codeaware_4bit.py \
    ...
    --include_code --code_max_lines 50 \
    --gpu_id 0 --top_k 100 --max_seq_length 768 --score_batch_size 8 2>&1 | tee logs/eval_pathswap_${TAG}_swap.log | tail -3
