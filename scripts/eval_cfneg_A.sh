#!/bin/bash
# Eval CF-Neg A reranker on graph, hybrid, BM25 pools
GPU_ID=${1:-4}
PYTHON=/home/chenlibin/miniconda3/envs/tgn/bin/python3
TEST_DATA=data/grepo_text/grepo_test.jsonl
MODEL_PATH=/data/shuyang/models/Qwen2.5-7B-Instruct
LORA_PATH=experiments/rankft_cfneg_A/final

echo "=== CF-Neg A Eval (GPU $GPU_ID) ==="
echo "Start: $(date)"

if [ ! -d "$LORA_PATH" ]; then
    LORA_PATH=experiments/rankft_cfneg_A/best
...
for pool_name in graph hybrid bm25; do
    ...
    CUDA_VISIBLE_DEVICES=$GPU_ID $PYTHON -u scripts/eval_rankft_4bit.py \
        ...
        --output_dir experiments/rankft_cfneg_A/eval_${pool_name} \
...
for pool in graph hybrid bm25; do
    f="experiments/rankft_cfneg_A/eval_${pool}/summary.json"
    if [ -f "$f" ]; then
