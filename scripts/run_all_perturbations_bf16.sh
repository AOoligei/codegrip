#!/bin/bash
# Run all path perturbation evals on a full GPU (bf16, batch=16)
# Usage: bash scripts/run_all_perturbations_bf16.sh [GPU_ID]
...
# Also run the original (non-perturbed) as bf16 baseline for apples-to-apples comparison
CONDITIONS="shuffle_dirs shuffle_filenames remove_module_names flatten_dirs swap_leaf_dirs"

echo "=== Path Perturbation Eval (bf16, GPU $GPU_ID) ==="
echo "Start: $(date)"
echo ""
...
for cond in $CONDITIONS; do
    OUT="experiments/path_perturb_${cond}/eval_bf16/summary.json"
    if [ -f "$OUT" ]; then
