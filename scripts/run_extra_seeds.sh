#!/bin/bash
# Run additional seeds (3, 4) for graph-hard vs bm25-only
# Usage: bash scripts/run_extra_seeds.sh <gpu_id>
set -e
GPU=$1
...
log() { echo "[$(date)] [SEEDS] $1" | tee -a orchestrate.log; }

for SEED in 3 4; do
    # Graph-hard
    EXP_G="experiments/rankft_runB_graph_seed${SEED}"
    if [ ! -d "${EXP_G}/final" ]; then
        log "Training graph-hard seed${SEED} on GPU ${GPU}..."
