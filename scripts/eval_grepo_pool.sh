...
for ckpt in $CKPTS; do
    i=$((i+1))
    NAME=$(basename "$ckpt")
    OUTDIR="${DIR}/eval_${NAME}_${POOL_NAME}"
    
    CUDA_VISIBLE_DEVICES=${GPU} $PYTHON scripts/eval_rankft_4bit.py \
        --model_path ${MODEL} --lora_path ${ckpt} \
        --test_data ${TEST} --bm25_candidates ${CAND} \
        --output_dir ${OUTDIR} --gpu_id 0 --top_k 200 --max_seq_length 512 --score_batch_size 2 \
        2>/dev/null
    ...
    echo "[${i}/${TOTAL}] ${NAME} ${POOL_NAME}: ${H1}%${IS_BEST}"
...
echo ">>> BEST ${POOL_NAME}: ${BEST_H1}% (${BEST_CKPT})"
echo "${POOL_NAME}:${BEST_H1}:${BEST_CKPT}" > "${DIR}/best_${POOL_NAME}.txt"
