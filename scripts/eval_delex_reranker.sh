#!/bin/bash
# Eval delex-trained reranker on all 3 pools: graph, hybrid, BM25
GPU_ID=${1:-3}
PYTHON=/home/chenlibin/miniconda3/envs/tgn/bin/python3
TEST_DATA=data/grepo_text/grepo_test.jsonl
MODEL_PATH=/data/shuyang/models/Qwen2.5-7B-Instruct
LORA_PATH=experiments/rankft_delex50/final

echo "=== Delex Reranker Eval (GPU $GPU_ID) ==="
echo "Start: $(date)"

# Use 'best' if 'final' doesn't exist yet
if [ ! -d "$LORA_PATH" ]; then
    LORA_PATH=experiments/rankft_delex50/best
...
CUDA_VISIBLE_DEVICES=$GPU_ID $PYTHON -u scripts/eval_rankft_4bit.py \
    ...
    --output_dir experiments/rankft_delex50/eval_bm25 \
    --gpu_id 0 --top_k 200 --max_seq_length 512 --score_batch_size 16
echo "  BM25 done: $(date)"
